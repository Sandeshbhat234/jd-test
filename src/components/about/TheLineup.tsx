...
export default function TheLineup() {
  ...
  // Navigation shared by the list, the dots and the arrows.
  ...
  return (
    <section
      ...
      <div
        ref={headerRef}
        className="absolute left-[clamp(20px,5.5vw,98px)] top-[clamp(48px,12vh,126px)] z-20 w-[min(86vw,681px)]">
      ...
      <div
        ref={listWrapRef}
        className="absolute left-[clamp(20px,5.5vw,98px)] top-[clamp(200px,34vh,371px)] z-20 flex flex-col gap-[clamp(6px,1.5vh,16px)]">
        {PRODUCTS.map((p, i) => (
          <button
            type="button"
            key={p.name}
      ...
      {PRODUCTS.map((p, i) => (
        <button
          ...
          onClick={() => goTo(i)}
          aria-label={p.name}
          className="absolute left-1/2 top-1/2 h-[clamp(240px,44vh,460px)] w-[clamp(220px,26vw,420px)] will-change-transform">
          <Image
            src={`${A5}/${p.img}`}
            alt={p.name}
            fill
      ...
      {/* pagination dots */}
      <div className="absolute bottom-[clamp(40px,9vh,105px)] left-1/2 z-20 flex -translate-x-1/2 items-center gap-[clamp(8px,1.3vw,19px)]">
        {PRODUCTS.map((p, i) => (
          <span
            key={p.name}
            ref={(el) => {
      ...
      {/* prev / next arrows */}
      <div className="absolute bottom-[clamp(36px,8vh,96px)] right-[clamp(20px,5vw,90px)] z-20 flex items-center gap-[15px]">
        <button
          type="button"
